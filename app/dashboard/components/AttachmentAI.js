...
export default function AttachmentAI({
  ...
  const removeFile = (fileName) => {
    ...
    const updatedAnalyses = analyses.filter(
      (analysis) => analysis.file?.name !== fileName
    );
                  ...
                  <div className="space-y-2">
                    ...
                    {item.analysis?.metadata && (
                      <details className="text-xs">
                        ...
                        <pre className="bg-gray-50 rounded p-2 overflow-auto max-h-40 text-[11px] text-gray-700">
                          {JSON.stringify(item.analysis.metadata, null, 2)}
                        </pre>
                      </details>
                    )}
